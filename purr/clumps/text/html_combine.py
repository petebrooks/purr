...
def html_to_markdown(html_content: str) -> str:
    ...
    body = soup.find('body')
    return md(str(body)) if body else md(html_content)

def process_file(file_path: str, minify: bool, convert_to_md: bool) -> str:
...
def combine_files(
    ...
    current_file_path = os.path.join(output_dir, f"combined_{current_file_num}.{'md' if convert_to_md else 'html'}")
        ...
        for subdir, dirs, files in os.walk(input_dir):
            for file in files:
                if file.endswith(".html"):
                    ...
                    futures.append(executor.submit(process_file, file_path, minify, convert_to_md))
